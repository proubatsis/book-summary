...
case class UserSummaryHistoryView(username: String, summaries: List[SummaryHistoryViewItem], navbar: NavbarView) extends PageView
...
object UserSummaryHistoryView {
  ...
  def fromUserAndSummaries(username: String, booksAndSummaries: List[(Book, BookSummary)], account: Option[Account]): UserSummaryHistoryView = {
    ...
    UserSummaryHistoryView(username, items, NavbarView.fromAccountOption(account))
  }
}
